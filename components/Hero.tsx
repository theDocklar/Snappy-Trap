"use client";

import Image from "next/image";
import { useState, useEffect, useRef } from "react";
import SocialIcons from "./SocialIcons";
import Link from "next/link";

export default function Hero() {
  const images = [
    {
      src: "/images/DK-110-Kitchen-Ambience-1920x1440px.jpg",
      alt: "Kitchen",
      title: "Kitchens",
      link: "/products/dk-110",
    },
    {
      src: "/images/DK-110-Kitchen-User-01-1920x1440px.jpg",
      alt: "Family Kitchen",
      title: "Kitchens",
      link: "/how-it-works",
    },
    {
      src: "/images/DK110-Kitchen-User-02-1920x1440px.jpg",
      alt: "Family Kitchen",
      title: "Kitchens",
      link: "/products/dk-110",
    },
    {
      src: "/images/DK110-Kitchen-SKU-Close-Up-1920x1440px.jpg",
      alt: "Kitchen",
      title: "Kitchens",
      link: "/products/dk-110",
    },
    {
      src: "/images/DK110-Kitchen-Packaging-1920x1440px.jpg",
      alt: "Kitchen",
      title: "Kitchens",
      link: "/products/dk-110",
    },
    {
      src: "/images/DK110-Kitchen-Close-Up-1920x1440px.jpg",
      alt: "Video",
      title: "Kitchens",
      link: "/how-it-works",
    },
    {
      src: "/images/DK-100-Minibar-Ambience-1920x1440px.jpg",
      alt: "Minibar",
      title: "Mini Bars",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Minibar-User-1920x1440px.jpg",
      alt: "Minibar",
      title: "Mini Bars",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Minibar-Close-Up-1920x1440px.jpg",
      alt: "Minibar",
      title: "Mini Bars",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Minibar-Packaging-2-1920x1440px.jpg",
      alt: "Minibar",
      title: "Mini Bars",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Minibar-Installation-1920x1440px.jpg",
      alt: "Minibar",
      title: "Mini Bars",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Laundry-Ambience-1920x1440px.jpg",
      alt: "Laundry",
      title: "Laundry Rooms",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Bathroom-User-1920x1440px.jpg",
      alt: "Laundry",
      title: "Laundry Rooms",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Laundry-Close-Up-1920x1440px.jpg",
      alt: "Laundry",
      title: "Laundry Rooms",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Laundry-Packaging-2-1920x1440px.jpg",
      alt: "Laundry",
      title: "Laundry Rooms",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-100-Laundry-Installation-1920x1440px.jpg",
      alt: "Laundry",
      title: "Laundry Rooms",
      link: "/products/dk-100",
    },
    {
      src: "/images/DK-105-Bathroom-Ambience-1920x1440px.jpg",
      alt: "Bathroom",
      title: "Bathrooms",
      link: "/products/dk-105",
    },
    {
      src: "/images/DK-105-Bathroom-User-1920x1440px.jpg",
      alt: "Bathroom",
      title: "Bathrooms",
      link: "/products/dk-105",
    },
    {
      src: "/images/DK-105-Bathroom-Close-Up-1920x1440px.jpg",
      alt: "Bathroom",
      title: "Bathrooms",
      link: "/products/dk-105",
    },
    {
      src: "/images/DK-105-Bathroom-Packaging-2-1920x1440px.jpg",
      alt: "Bathroom",
      title: "Bathrooms",
      link: "/products/dk-105",
    },
    {
      src: "/images/DK-105-Bathroom-Installation-1920x1440px.jpg",
      alt: "Bathroom",
      title: "Bathrooms",
      link: "/products/dk-105",
    },

    // {
    //   src: "/images/dk110-2.avif",
    //   alt: "double kitchen sink",
    //   title: "Double Kitchen Sinks",
    //   link: "/products/dk-110",
    // },
    // {
    //   src: "/images/DK-100 DE.jpg",
    //   alt: "DK-100",
    //   title: "Single Kitchen Sinks",
    //   link: "/products/dk-100",
    // },
    // {
    //   src: "/images/hero1.jpg",
    //   alt: "Bathroom",
    //   title: "Bathroom Sinks",
    //   link: "/products",
    // },
    // //
    // {
    //   src: "/images/DK-105 EXTENSION.jpg",
    //   alt: "DK-105",
    //   title: "Bathroom Sinks",
    //   link: "/products/dk-105",
    // },
    // {
    //   src: "/images/Laundry Room.jpg",
    //   alt: "Laundry Room",
    //   title: "Laundry Room",
    //   link: "/products",
    // },
    // {
    //   src: "/images/DK-100 DE.jpg",
    //   alt: "DK-100",
    //   title: "Laundry Tubs & Sinks",
    //   link: "/products/dk-100",
    // },
    // // {
    // //   src: "/images/Wet Bar Sink.jpg",
    // //   alt: "Bar Sink",
    // //   title: "Prep & Bar Areas",
    // // },
    // {
    //   src: "/images/DK-100 DE.jpg",
    //   alt: "DK-100",
    //   title: "Prep & Bar Sinks",
    //   link: "/products/dk-100",
    // },
  ];

  const [currentIndex, setCurrentIndex] = useState(0);
  const [isAnimating, setIsAnimating] = useState(false);
  const isAnimatingRef = useRef(false);

  const nextIndex = (currentIndex + 1) % images.length;

  const slide = () => {
    if (isAnimatingRef.current) return;
    isAnimatingRef.current = true;
    setIsAnimating(true);
  };

  const onTransitionEnd = () => {
    // Instantly reset: move next image into current slot, clear animation
    setCurrentIndex((prev) => (prev + 1) % images.length);
    setIsAnimating(false);
    isAnimatingRef.current = false;
  };

  useEffect(() => {
    const interval = setInterval(slide, 4000);
    return () => clearInterval(interval);
  }, []);

  return (
    <section className="bg-gradient-to-br from-[#002D72] via-[#003A8C] to-[#0056FF] pt-24 pb-12 sm:pt-28 sm:pb-16 lg:pt-40 lg:pb-32 relative overflow-hidden min-h-screen flex items-center">
      <div className="absolute inset-0 opacity-5" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12 items-start">
          {/* Left Side - Image */}
          <div className="relative order-1 lg:order-1">
            <div className="relative rounded-[35px] sm:rounded-[50px] lg:rounded-[65px] overflow-hidden aspect-[4/3] w-full">
              {/*
                A 200%-wide track holding [current | next] side by side.
                When animating, slide it left by 50% of the track (= 100% of viewport).
                onTransitionEnd: swap indexes and snap back with no transition.
              */}
              <div
                className="absolute inset-0 flex h-full"
                style={{
                  width: "200%",
                  transform: isAnimating
                    ? "translateX(-50%)"
                    : "translateX(0%)",
                  transition: isAnimating
                    ? "transform 0.75s ease-in-out"
                    : "none",
                }}
                onTransitionEnd={onTransitionEnd}
              >
                {/* Current image — left half */}
                <div
                  className="relative h-full flex-shrink-0"
                  style={{ width: "50%" }}
                >
                  <Image
                    src={images[currentIndex].src}
                    alt={images[currentIndex].alt}
                    fill
                    className="object-cover"
                    quality={60}
                    priority
                  />
                </div>

                {/* Next image — right half, always pre-loaded */}
                <div
                  className="relative h-full flex-shrink-0"
                  style={{ width: "50%" }}
                >
                  <Image
                    src={images[nextIndex].src}
                    alt={images[nextIndex].alt}
                    fill
                    className="object-cover"
                    quality={60}
                  />
                </div>
              </div>

              {/* Feature tag (top-left) */}
              <div className="absolute top-5 left-5 z-20">
                <div className="bg-[#012965] text-white font-bold rounded-full px-3 sm:px-4 py-2 shadow-md text-sm sm:text-base">
                  {images[currentIndex].title}
                </div>
              </div>

              {/* Learn More Button */}
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
                <Link href={images[currentIndex].link || "/products"}>
                  <button
                    className="bg-white/90 hover:bg-white text-[#012965] px-6 py-3 rounded-full font-bold text-sm sm:text-base flex items-center gap-2 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                    aria-label="Learn more about SnappyTrap"
                  >
                    Learn More
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={2.5}
                      stroke="currentColor"
                      className="w-4 h-4"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"
                      />
                    </svg>
                  </button>
                </Link>
              </div>
            </div>

            {/* Brand Slogan */}
            <div className="space-y-3 sm:space-y-4">
              <h1 className="font-black mt-8 sm:mt-10 text-center text-3xl sm:text-4xl md:text-4xl lg:text-5xl text-white tracking-[-1.8px] leading-tight">
                Fast, Flexible,
                <br />
                Flawless Installation
              </h1>
              <p className="font-black text-center text-2xl sm:text-3xl lg:text-4xl xl:text-5xl text-white tracking-[0.75px]">
                Sink Drains Made Easy
              </p>
            </div>
          </div>

          {/* Right Side - Content */}
          <div className="order-2 lg:order-2 space-y-5 sm:space-y-6">
            <div className="-mt-6 sm:-mt-8 lg:-mt-12 -mb-6 sm:-mb-8 lg:-mb-12 w-full flex justify-center">
              <div className="w-[75%] sm:w-[70%] lg:w-[90%]">
                <Image
                  src="/images/snappy-red.png"
                  alt="SnappyTrap"
                  width={2844}
                  height={468}
                  className="w-full h-auto object-contain"
                />
              </div>
            </div>

            {/* Feature Pills — packaging benefit bullets */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-w-xl mx-auto lg:mx-0">
              <div className="flex items-center space-x-2 sm:space-x-3 sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4">
                <div className="relative h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 flex items-center justify-center flex-shrink-0">
                  <Image
                    src="/images/snapping.png"
                    alt="Easy installation"
                    width={70}
                    height={70}
                    className="object-contain"
                  />
                </div>
                <span className="font-bold text-sm sm:text-base text-white tracking-[0.35px] uppercase">
                  Easy Installation
                </span>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-3 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4">
                <div className="relative h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 flex items-center justify-center flex-shrink-0">
                  <Image
                    src="/images/100.png"
                    alt="Complete kits"
                    width={70}
                    height={70}
                    className="object-contain"
                  />
                </div>
                <span className="font-bold text-sm sm:text-base text-white tracking-[0.35px] uppercase">
                  Complete Kits
                </span>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-3 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4">
                <div className="relative h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 flex items-center justify-center flex-shrink-0">
                  <Image
                    src="/images/no-tools.png"
                    alt="Tool-free"
                    width={70}
                    height={70}
                    className="object-contain"
                  />
                </div>
                <span className="font-bold text-sm sm:text-base text-white tracking-[0.35px] uppercase">
                  Tool-Free
                </span>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-3 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4">
                <div className="relative h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 flex items-center justify-center flex-shrink-0">
                  <Image
                    src="/images/kitchen-sink.png"
                    alt="Kitchens, baths and more"
                    width={70}
                    height={70}
                    className="object-contain"
                  />
                </div>
                <span className="font-bold text-sm sm:text-base text-white tracking-[0.35px] uppercase">
                  Kitchens, Baths & More
                </span>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-3 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4">
                <div className="relative h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 flex items-center justify-center flex-shrink-0">
                  <Image
                    src="/images/fit-anywhere.png"
                    alt="Fits any space"
                    width={70}
                    height={70}
                    className="object-contain"
                  />
                </div>
                <span className="font-bold text-sm sm:text-base text-white tracking-[0.35px] uppercase">
                  Fits any Space
                </span>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-3 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4">
                <div className="relative h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 flex items-center justify-center flex-shrink-0">
                  <Image
                    src="/images/complete-kits.png"
                    alt="Code approved"
                    width={70}
                    height={70}
                    className="object-contain"
                  />
                </div>
                <span className="font-bold text-sm sm:text-base text-white tracking-[0.35px] uppercase">
                  Code Approved
                </span>
              </div>
            </div>

            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center lg:justify-start">
              <Link
                href="/how-it-works"
                className="bg-[#043687] hover:bg-[#0056FF] text-white px-6 sm:px-8 lg:px-10 py-4 sm:py-5 lg:py-6 text-base sm:text-lg lg:text-xl font-bold rounded-xl sm:rounded-2xl tracking-wide uppercase text-center ring-1 ring-white/25 shadow-[0_10px_30px_rgba(0,0,0,0.35),0_0_0_1px_rgba(255,255,255,0.08)_inset] hover:shadow-[0_16px_40px_rgba(0,0,0,0.45),0_0_0_1px_rgba(255,255,255,0.12)_inset] transform hover:-translate-y-1 sm:hover:-translate-y-2 transition-all duration-300"
              >
                Installation Help
              </Link>
              <a
                href="#shop"
                className="bg-[#043687] hover:bg-[#0056FF] text-white px-6 sm:px-8 lg:px-10 py-4 sm:py-5 lg:py-6 text-base sm:text-lg lg:text-xl font-black rounded-xl sm:rounded-2xl tracking-wide uppercase text-center ring-1 ring-white/25 shadow-[0_10px_30px_rgba(0,0,0,0.35),0_0_0_1px_rgba(255,255,255,0.08)_inset] hover:shadow-[0_16px_40px_rgba(0,0,0,0.45),0_0_0_1px_rgba(255,255,255,0.12)_inset] transform hover:-translate-y-1 sm:hover:-translate-y-2 transition-all duration-300"
              >
                SHOP now
              </a>
            </div>

            {/* Reviews & Warranty */}
            <div className="flex flex-col sm:flex-row items-center lg:items-start sm:items-center justify-center lg:justify-start gap-4 sm:gap-8 lg:gap-14 text-blue-100">
              <div className="flex items-center justify-center lg:justify-start space-x-2">
                <svg
                  width="122"
                  height="16"
                  viewBox="0 0 122 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-3 sm:h-4"
                >
                  <path
                    d="M9.80808 1.04903C9.84936 0.984264 9.91314 0.929745 9.9922 0.891628C10.0713 0.853511 10.1625 0.833313 10.2556 0.833313C10.3486 0.833313 10.4398 0.853511 10.5189 0.891628C10.598 0.929745 10.6617 0.984264 10.703 1.04903L12.8792 4.47178C13.0225 4.69706 13.2342 4.89196 13.4959 5.03976C13.7576 5.18756 14.0616 5.28384 14.3818 5.32033L19.2484 5.87336C19.3406 5.88373 19.4273 5.91393 19.4985 5.96055C19.5698 6.00717 19.6228 6.06834 19.6516 6.13714C19.6804 6.20595 19.6839 6.27964 19.6616 6.34989C19.6393 6.42014 19.5921 6.48413 19.5254 6.53464L16.0059 9.19588C15.7737 9.37152 15.6001 9.58833 15.4998 9.82764C15.3995 10.067 15.3757 10.3216 15.4303 10.5697L16.2612 14.3296C16.2774 14.4012 16.2675 14.4749 16.2324 14.5423C16.1974 14.6097 16.1386 14.668 16.0629 14.7108C15.9872 14.7535 15.8975 14.7788 15.8041 14.7838C15.7108 14.7889 15.6174 14.7734 15.5348 14.7393L11.1844 12.9632C10.8978 12.8463 10.5788 12.7852 10.2551 12.7852C9.93132 12.7852 9.6124 12.8463 9.32574 12.9632L4.97628 14.7393C4.89369 14.7732 4.80049 14.7885 4.70728 14.7834C4.61406 14.7782 4.52458 14.7529 4.44901 14.7102C4.37343 14.6675 4.3148 14.6092 4.27978 14.5419C4.24476 14.4747 4.23475 14.4011 4.2509 14.3296L5.08085 10.5704C5.13567 10.3222 5.11192 10.0674 5.01164 9.82794C4.91136 9.58848 4.73757 9.37156 4.50525 9.19588L0.985737 6.53537C0.918469 6.48492 0.8708 6.42082 0.848162 6.35036C0.825523 6.27989 0.828826 6.20591 0.857692 6.13684C0.886558 6.06776 0.939828 6.00637 1.01143 5.95966C1.08304 5.91295 1.1701 5.88279 1.2627 5.87262L6.12841 5.32033C6.44895 5.28412 6.75336 5.18797 7.01544 5.04015C7.27751 4.89234 7.4894 4.69728 7.63287 4.47178L9.80808 1.04903Z"
                    fill="#FCC800"
                    stroke="#FCC800"
                    strokeWidth="1.66667"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M44.8524 1.04903C44.8937 0.984264 44.9575 0.929745 45.0366 0.891628C45.1156 0.853511 45.2069 0.833313 45.2999 0.833313C45.393 0.833313 45.4842 0.853511 45.5633 0.891628C45.6423 0.929745 45.7061 0.984264 45.7474 1.04903L47.9235 4.47178C48.0669 4.69706 48.2785 4.89196 48.5402 5.03976C48.802 5.18756 49.1059 5.28384 49.4261 5.32033L54.2928 5.87336C54.385 5.88373 54.4716 5.91393 54.5429 5.96055C54.6141 6.00717 54.6672 6.06834 54.696 6.13714C54.7248 6.20595 54.7283 6.27964 54.7059 6.34989C54.6836 6.42014 54.6365 6.48413 54.5697 6.53464L51.0502 9.19588C50.8181 9.37152 50.6445 9.58833 50.5442 9.82764C50.4439 10.067 50.42 10.3216 50.4746 10.5697L51.3055 14.3296C51.3218 14.4012 51.3118 14.4749 51.2768 14.5423C51.2417 14.6097 51.183 14.668 51.1073 14.7108C51.0315 14.7535 50.9419 14.7788 50.8485 14.7838C50.7551 14.7889 50.6618 14.7734 50.5792 14.7393L46.2288 12.9632C45.9421 12.8463 45.6232 12.7852 45.2995 12.7852C44.9757 12.7852 44.6568 12.8463 44.3701 12.9632L40.0207 14.7393C39.9381 14.7732 39.8449 14.7885 39.7516 14.7834C39.6584 14.7782 39.569 14.7529 39.4934 14.7102C39.4178 14.6675 39.3592 14.6092 39.3242 14.5419C39.2891 14.4747 39.2791 14.4011 39.2953 14.3296L40.1252 10.5704C40.18 10.3222 40.1563 10.0674 40.056 9.82794C39.9557 9.58848 39.7819 9.37156 39.5496 9.19588L36.0301 6.53537C35.9628 6.48492 35.9152 6.42082 35.8925 6.35036C35.8699 6.27989 35.8732 6.20591 35.9021 6.13684C35.9309 6.06776 35.9842 6.00637 36.0558 5.95966C36.1274 5.91295 36.2145 5.88279 36.3071 5.87262L41.1728 5.32033C41.4933 5.28412 41.7977 5.18797 42.0598 5.04015C42.3219 4.89234 42.5338 4.69728 42.6772 4.47178L44.8524 1.04903Z"
                    fill="#FCC800"
                    stroke="#FCC800"
                    strokeWidth="1.66667"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M79.8969 1.04903C79.9382 0.984264 80.0019 0.929745 80.081 0.891628C80.1601 0.853511 80.2513 0.833313 80.3444 0.833313C80.4374 0.833313 80.5286 0.853511 80.6077 0.891628C80.6868 0.929745 80.7506 0.984264 80.7918 1.04903L82.968 4.47178C83.1113 4.69706 83.323 4.89196 83.5847 5.03976C83.8464 5.18756 84.1504 5.28384 84.4706 5.32033L89.3372 5.87336C89.4294 5.88373 89.5161 5.91393 89.5873 5.96055C89.6586 6.00717 89.7116 6.06834 89.7404 6.13714C89.7692 6.20595 89.7727 6.27964 89.7504 6.34989C89.7281 6.42014 89.6809 6.48413 89.6142 6.53464L86.0947 9.19588C85.8626 9.37152 85.6889 9.58833 85.5886 9.82764C85.4884 10.067 85.4645 10.3216 85.5191 10.5697L86.35 14.3296C86.3662 14.4012 86.3563 14.4749 86.3212 14.5423C86.2862 14.6097 86.2274 14.668 86.1517 14.7108C86.076 14.7535 85.9863 14.7788 85.8929 14.7838C85.7996 14.7889 85.7062 14.7734 85.6236 14.7393L81.2732 12.9632C80.9866 12.8463 80.6677 12.7852 80.3439 12.7852C80.0201 12.7852 79.7012 12.8463 79.4146 12.9632L75.0651 14.7393C74.9825 14.7732 74.8893 14.7885 74.7961 14.7834C74.7029 14.7782 74.6134 14.7529 74.5378 14.7102C74.4622 14.6675 74.4036 14.6092 74.3686 14.5419C74.3336 14.4747 74.3236 14.4011 74.3397 14.3296L75.1697 10.5704C75.2245 10.3222 75.2007 10.0674 75.1004 9.82794C75.0002 9.58848 74.8264 9.37156 74.5941 9.19588L71.0745 6.53537C71.0073 6.48492 70.9596 6.42082 70.937 6.35036C70.9143 6.27989 70.9176 6.20591 70.9465 6.13684C70.9754 6.06776 71.0286 6.00637 71.1002 5.95966C71.1718 5.91295 71.2589 5.88279 71.3515 5.87262L76.2172 5.32033C76.5378 5.28412 76.8422 5.18797 77.1042 5.04015C77.3663 4.89234 77.5782 4.69728 77.7217 4.47178L79.8969 1.04903Z"
                    fill="#FCC800"
                    stroke="#FCC800"
                    strokeWidth="1.66667"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M110.419 1.04903C110.461 0.984264 110.525 0.929745 110.604 0.891628C110.683 0.853511 110.774 0.833313 110.867 0.833313C110.96 0.833313 111.051 0.853511 111.13 0.891628C111.209 0.929745 111.273 0.984264 111.314 1.04903L113.491 4.47178C113.634 4.69706 113.846 4.89196 114.107 5.03976C114.369 5.18756 114.673 5.28384 114.993 5.32033L119.86 5.87336C119.952 5.88373 120.039 5.91393 120.11 5.96055C120.181 6.00717 120.234 6.06834 120.263 6.13714C120.292 6.20595 120.295 6.27964 120.273 6.34989C120.251 6.42014 120.203 6.48413 120.137 6.53464L116.617 9.19588C116.385 9.37152 116.211 9.58833 116.111 9.82764C116.011 10.067 115.987 10.3216 116.042 10.5697L116.873 14.3296C116.889 14.4012 116.879 14.4749 116.844 14.5423C116.809 14.6097 116.75 14.668 116.674 14.7108C116.599 14.7535 116.509 14.7788 116.416 14.7838C116.322 14.7889 116.229 14.7734 116.146 14.7393L111.796 12.9632C111.509 12.8463 111.19 12.7852 110.866 12.7852C110.543 12.7852 110.224 12.8463 109.937 12.9632L105.588 14.7393C105.505 14.7732 105.412 14.7885 105.319 14.7834C105.225 14.7782 105.136 14.7529 105.06 14.7102C104.985 14.6675 104.926 14.6092 104.891 14.5419C104.856 14.4747 104.846 14.4011 104.862 14.3296L105.692 10.5704C105.747 10.3222 105.723 10.0674 105.623 9.82794C105.523 9.58848 105.349 9.37156 105.117 9.19588L101.597 6.53537C101.53 6.48492 101.482 6.42082 101.46 6.35036C101.437 6.27989 101.44 6.20591 101.469 6.13684C101.498 6.06776 101.551 6.00637 101.623 5.95966C101.694 5.91295 101.781 5.88279 101.874 5.87262L106.74 5.32033C107.06 5.28412 107.365 5.18797 107.627 5.04015C107.889 4.89234 108.101 4.69728 108.244 4.47178L110.419 1.04903Z"
                    fill="#FCC800"
                    stroke="#FCC800"
                    strokeWidth="1.66667"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
                <span className="font-bold text-lg sm:text-xl lg:text-2xl text-center lg:text-left">
                  Reviews
                </span>
              </div>
              <div className="flex items-center justify-center lg:justify-start space-x-3 sm:space-x-4 lg:space-x-6">
                <svg
                  width="48"
                  height="42"
                  viewBox="0 0 48 42"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-10 h-8 sm:w-12 sm:h-9 lg:w-14 lg:h-10"
                >
                  <path
                    d="M34.6908 22.6468C34.6908 30.8178 28.2696 34.9033 20.6377 37.2728C20.238 37.3935 19.8039 37.3877 19.4085 37.2565C11.7582 34.9033 5.33704 30.8178 5.33704 22.6468V11.2074C5.33704 10.774 5.53032 10.3584 5.87438 10.0519C6.21844 9.74541 6.68508 9.57324 7.17164 9.57324C10.8409 9.57324 15.4274 7.61221 18.6196 5.12823C19.0083 4.83244 19.5027 4.66992 20.0139 4.66992C20.5251 4.66992 21.0195 4.83244 21.4082 5.12823C24.6188 7.62855 29.1869 9.57324 32.8561 9.57324C33.3427 9.57324 33.8094 9.74541 34.1534 10.0519C34.4975 10.3584 34.6908 10.774 34.6908 11.2074V22.6468Z"
                    stroke="#BEDBFF"
                    strokeWidth="1.49896"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
                <span className="font-bold text-base sm:text-lg lg:text-xl text-center lg:text-left">
                  5 year Limited warranty
                </span>
              </div>
            </div>

            {/* Social Media Icons */}
            <div className="flex items-center justify-center lg:justify-start space-x-8 mt-8">
              <a
                href="https://youtube.com/@snappytrap"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 hover:opacity-70 transition-opacity"
                aria-label="YouTube"
              >
                <SocialIcons platform="YouTube" color="Negative" />
              </a>
              <a
                href="https://instagram.com/snappytrap"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 hover:opacity-70 transition-opacity"
                aria-label="Instagram"
              >
                <SocialIcons platform="Instagram" color="Negative" />
              </a>
              <a
                href="https://facebook.com/snappytrap"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 hover:opacity-70 transition-opacity"
                aria-label="Facebook"
              >
                <SocialIcons platform="Facebook" color="Negative" />
              </a>
              <a
                href="https://tiktok.com/@snappytrap"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 hover:opacity-70 transition-opacity"
                aria-label="TikTok"
              >
                <SocialIcons platform="TikTok" color="Negative" />
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
